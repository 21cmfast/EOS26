...
def setup_logging(log_file: str) -> logging.Logger:
    """Configure the 21cmFAST logger with rich memory/time display, writing to log_file."""

    # Console pointed at the log file.
    # no_color=True strips ANSI escape codes so the file stays human-readable.
    # force_terminal=False is redundant for a file but makes intent explicit.
    os.makedirs(os.path.dirname(os.path.abspath(log_file)), exist_ok=True)  # ← here
    ...
    handler = RicherHandler(
        console=console,
        rich_tracebacks=True,
        tracebacks_show_locals=True,
        show_path=False,
        show_time_as_diff=True,       # elapsed time since first log line
        show_mem_usage=True,
        mem_backend="psutil",         # RSS; tracemalloc only tracks Python allocs
    )
