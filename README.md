...
RSS with `psutil`, and writes a phase-by-phase JSON record under
...
completed `HII_DIM` runs, run `uv run --no-sync scaling/run_scalingrelation.py
--update-readme`; it relabels the two measured columns, preserves the SU and
production-extrapolation cells, and fits the extrapolation reports using every
available scaling result.
